...
async function createJWT(account: string, user: string, privateKeyDer: Uint8Array, publicKeyFingerprint: string): Promise<string> {
...
serve(async (req) => {
  ...
  try {
    ...
    const privateKeyBase64 = Deno.env.get("PRIVATE_KEY_PATH")?.trim();
    const configuredFingerprint = Deno.env.get("SNOWFLAKE_PUBLIC_KEY_FP")?.trim();
    if (!account || !user || !privateKeyBase64) {
      return new Response(
        JSON.stringify({
          error: "Snowflake credentials not configured (need SF_ACCOUNT, SF_USER, and PRIVATE_KEY_PATH with base64-encoded PKCS#8 DER private key).",
        }),
        {
          status: 500,
          headers: {
