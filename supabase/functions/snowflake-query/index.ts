...
function buildJwtClaims(account: string, user: string, publicKeyFingerprintRaw: string) {
  const accountUpper = account.toUpperCase();
  const userUpper = user.toUpperCase();

  // Allow either:
  // - "SHA256:<base64>"  (from DESCRIBE USER)
  // - "<base64>"         (user pasted manually)
  let fp = publicKeyFingerprintRaw.trim();
  if (fp.toUpperCase().startsWith("SHA256:")) {
    fp = fp.slice("SHA256:".length);
  }

  const iss = `${accountUpper}.${userUpper}.SHA256:${fp}`;
  const sub = `${accountUpper}.${userUpper}`;
...
async function createJWT(
  ...
  publicKeyFingerprintRaw: string, // may be "SHA256:xxx" or just "xxx"
): Promise<string> {
  const { iss, sub } = buildJwtClaims(account, user, publicKeyFingerprintRaw);
